...
public:
    ...
    void apply_telemetry(const pypilot_servo_protocol::Telemetry& telemetry,
                         pypilot_data_model::DataModel<Real>& model,
                         uint64_t now_us) const {
        ServoTelemetrySample<Real> sample = make_sample(telemetry, now_us, model.rudder.range_deg.value);
        SensorDataModelWriter<Real>().write_servo(model, sample);
    }
};
